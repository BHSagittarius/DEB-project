...
function blockHtml(block){
  ...
  if(block.type === 'meeting'){
    return `
      <div class="block block-meeting" data-bid="${block.id}">
        <div class="block-actions"><button class="del-small" data-delblock="${block.id}">✕</button></div>
        <div class="meeting-edit">
          <input type="date" data-field="date" value="${esc(block.date || '')}">
          <textarea data-field="points" placeholder="Points discutés…">${esc(block.points || '')}</textarea>
          <textarea data-field="decisions" placeholder="Décisions / actions à suivre…">${esc(block.decisions || '')}</textarea>
        </div>
      </div>`;
  }
  figCounter++;
  ...
  box.innerHTML = notes.map(n => `
    <div class="note-card" data-id="${n.id}">
      ...
      <div class="add-block-row">
        ...
        <button class="btn ghost small" data-addblock="meeting">+ Réunion</button>
      </div>
    </div>
  `).join('');
}
...
document.getElementById('noteList').addEventListener('click', async e => {
  ...
  if(e.target.dataset.addblock){
    ...
    n.blocks.push(
      ...
      type === 'meeting' ? {id:uid(), type:'meeting', date:'', points:'', decisions:''} :
      {id:uid(), type:'image', url:'', caption:'', scale:''}
    );
...
async function init(){
  ...
  if(!notes.length){
    const alreadySeeded = await safeGet(STORE_SEED, true);
    if(!alreadySeeded){
      notes = [
        { id: uid(), title: "Comptes-rendus de réunion", blocks: [], updatedBy: myName, updatedAt: now() },
        { id: uid(), title: "Cadrage du projet", blocks: [
            { id: uid(), type: 'text', content: "# Problématique\n\n# Objectifs\n\n# Hypothèses\n\n# Protocole envisagé" }
          ], updatedBy: myName, updatedAt: now() },
      ];
      await safeSet(STORE_NOTES, notes, true);
      await safeSet(STORE_SEED, true, true);
    }
  }
  renderNotes();
}
