...
/* ---------- Stockage ----------
   ...
   1) window.storage — n'existe que dans l'aperçu Artifact de Claude.
   2) Firestore (Firebase) — vrai backend partagé entre tous les appareils,
      une fois firebase-config.js rempli avec la config du projet.
   3) localStorage — repli local, propre à chaque navigateur, utilisé tant
      que la config Firebase n'est pas renseignée (pas de synchronisation
      entre toi et ton binôme dans ce cas). */
const hasArtifactStorage = !!(window.storage && typeof window.storage.get === 'function');

let db = null;
if(!hasArtifactStorage && window.firebase && window.firebaseConfig && window.firebaseConfig.apiKey && window.firebaseConfig.apiKey !== 'REMPLACE_MOI'){
  try{
    firebase.initializeApp(window.firebaseConfig);
    db = firebase.firestore();
  } catch(e){ console.error('Initialisation Firebase impossible', e); db = null; }
}
const hasSharedBackend = hasArtifactStorage || !!db;
...
async function safeGet(key, shared){
  try{
    ...
    if(shared && db){
      const doc = await db.collection('carnet_projet').doc(key).get();
...
async function safeSet(key, value, shared){
  try{
    ...
    if(shared && db){
      await db.collection('carnet_projet').doc(key).set({ value, updatedAt: new Date().toISOString() });
      return;
    }
...
function renderWho(){
  ...
  box.innerHTML = `Connecté·e comme<br><strong>${esc(myName)}</strong> <button id="changeNameBtn">changer</button>
    ${!hasSharedBackend ? '<div class="local-note">stockage local à ce navigateur — configure Firebase (firebase-config.js) pour synchroniser entre vous</div>' : ''}`;
